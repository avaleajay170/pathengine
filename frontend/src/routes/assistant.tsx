...
function AssistantPage() {
  ...
  const gaps = Object.entries(profile.skillLevels)
    .map(([skill, value]) => ({ skill, current: Number(value) || 0, target: 80 }))
    ...
    <main className="mx-auto flex h-[calc(100vh-4rem)] max-w-7xl gap-6 px-4 py-6 sm:px-6">
      ...
      <aside className="hidden w-72 shrink-0 space-y-5 overflow-y-auto pb-2 lg:block">
        ...
        <div className="surface-card p-5">
          ...
          <p className="mt-2 text-sm">{profile.goal || "Complete onboarding to set a goal."}</p>
        ...
        <div className="surface-card p-5">
          ...
          <ul className="mt-3 space-y-2">
            {profile.completedCourses.map((id) => {
        ...
        <header className="flex items-center gap-3 border-b border-border p-4">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-muted-foreground">
              Reasoning over your goal, {Object.keys(profile.skillLevels).length} tracked skills and{" "}
